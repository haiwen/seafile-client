...
int delete_dir_recursively(const QString& path_in)
{
    ...
    std::unique_ptr<wchar_t[]> wpath(new wchar_t[len + 2]);

    wcscpy(wpath.get(), path.toStdWString().c_str());
    wpath[len + 1] = L'\0';

    SHFILEOPSTRUCTW fileop {};
    fileop.wFunc  = FO_DELETE;   // delete operation
    fileop.pFrom  = wpath.get(); // source file name as double null terminated string
    /* This is equivalent to FOF_SILENT | FOF_NOCONFIRMATION | FOF_NOERRORUI |
     * FOF_NOCONFIRMMKDIR. see https://docs.microsoft.com/en-us/windows/win32/api/shellapi/ns-shellapi-shfileopstructw */
    fileop.fFlags = FOF_NO_UI;   // do not prompt the user

    /* TODO: This function has been replaced in Windows Vista by IFileOperation. */
