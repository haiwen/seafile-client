...
void LocalRepo::translateSyncErrDetail(const QString &err_detail)
{
    if (err_detail == "Permission denied on server") {
        sync_error_detail = QObject::tr("Permission denied on server. Please try and resync the library");
        ...
        sync_error_detail = QObject::tr("Internal data corrupt on the client. Please try and resync the library");
